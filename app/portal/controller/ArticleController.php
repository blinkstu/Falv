...
class ArticleController extends HomeBaseController
{
    public function index()
    {
        ...
        $selected = Db::name('portal_category')->where('id', $categoryId)->find();
        if($selected == null){
            $selected = Db::name('portal_category')->where('id', $categories[0]->id)->find();
        }
        $path = $selected['path'];
        $path = explode("-", $path);
        $topId = $path[1];
        ...
        $categoryTree = $portalCategoryModel->adminCategoryTreeFrontend($categoryId, $topId);

        $this->assign('topCategory', $topCategory);
        $this->assign('category', $selected);
