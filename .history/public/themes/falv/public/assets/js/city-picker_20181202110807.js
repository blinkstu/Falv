...
(function($, window) {
  ...
  var defaults = {
    dataJson: null,
    selectpattern: [
      { field: "userProvinceId", placeholder: "选择省份" },
      { field: "userCityId", placeholder: "选择城市" },
      { field: "userDistrictId", placeholder: "选择区县" },
      { field: "userStreet", placeholder: "选择街道" }
    ],
    shorthand: false,
    storage: true,
    autoSelected: true,
    ...
    streetUrl: "http://passer-by.com/data_location/town/{json}.json",
    level: 3,
  ...
  var effect = {
    montage: function(data, pid, reg) {
      ...
      for (var i = 0; i < leng; i++) {
        if (data[i].parentId === pid) {
          //判断是否要输出区号
          code =
            config.code && data[i].cityCode !== ""
              ? "data-code=" + data[i].cityCode
              : "";
    ...
    seTemplet: function() {
      ...
      for (var i = 0; i < config.level; i++) {
        ...
        if (config.renderMode) {
          //模拟
          selectemplet +=
            '<div class="selector-item storey ' +
            citygrade +
            '" data-index="' +
            i +
            '">' +
            '<a href="javascript:;" class="selector-name reveal df-color ' +
            forbid +
            '">' +
            placeholder +
            "</a>" +
            ...
            field +
            '" class="input-price val-error" value="" data-required="' +
            field +
            '">' +
            '<div class="selector-list listing">' +
            searchStr +
            "<ul></ul></div>" +
            "</div>";
        } else {
    ...
    obtain: function(event) {
      var self = this,
        ...
        placeholder =
          config.selectpattern[index < 3 ? index + 1 : index].placeholder,
        placeholderStr = !config.renderMode
          ? '<option class="caller" value="">' +
            placeholder +
            "</option>" +
      ...
      $selector.trigger("choose-" + grade[index] + ".citypicker", [
        $target,
        values
      ]);
      //赋值给隐藏域-区号
    ...
    search: function(event) {
      ...
      if (keycode !== 13 && keycode !== 38 && keycode !== 40) {
        $.each(this.options.dataJson, function(key, value) {
          //拼音或者名称搜索
          if (
            value.pinyin.toLocaleLowerCase().search(inputVal) > -1 ||
            value.name.search(inputVal) > -1 ||
            value.id.search(inputVal) > -1
          ) {
            result.push(value);
          }
        });
        ...
        $parent
          ...
          .html(
            htmls
              ? htmls
              : "<li>" +
                  self.options.searchNotStr.replace(
                    "{city}",
    ...
    operation: function(event) {
      ...
      var $target = $(event.target),
        $sibl = $target.hasClass("input-search")
          ? $target.parents(".listing")
          : $target.siblings(".listing"),
    ...
    evaluation: function(arr, arrayVal) {
      ...
      $.each(arr, function(item, value) {
        ...
        if (config.renderMode) {
          ...
          $forward
            .find("ul")
            .html(effect.montage.apply(self, [config.dataJson, value.id]));
          $original
            .find('.caller[data-id="' + value.id + '"]')
            .addClass("active");
        } else {
          $forward.html(
            effect.montage.apply(self, [config.dataJson, value.id])
          );
          $original
            .find('.caller[data-id="' + value.id + '"]')
            .prop("selected", true);
  ...
  Citypicker.prototype = {
    init: function() {
      var self = this,
        config = self.options,
        code = config.code
          ? '<input type="hidden" role="code" name="' +
            config.code +
            '" value="">'
          : "";
      //是否开启存储区号，是就加入一个隐藏域
      ...
      if (config.renderMode) {
        ...
        $selector
          ...
          .eq(0)
          .find("ul")
          .html(effect.montage.apply(self, [config.dataJson, "100000"]));
      } else {
        //原生>添加数据
        $selector
          .find(".province")
          .append(effect.montage.apply(self, [config.dataJson, "100000"]));
      }
      //初始化后的回调函数
    ...
    setCityVal: function(val) {
      ...
      // 反向排序数组
      resultArray =
        result[0].parentId === "100000" ? result.sort() : result.reverse();
      // 设置默认值
    ...
    changeStatus: function(status) {
      ...
      if (status === "disabled") {
        ...
        !config.renderMode
          ? self.$selector.find("select").prop("disabled", true)
          : "";
      } else if (status === "current") {
        self.$selector
          .find(".reveal")
          .removeClass("disabled forbid")
          .siblings(".input-price")
          .prop("disabled", false);

        !config.renderMode
          ? self.$selector.find("select").prop("disabled", false)
          : "";
      }
    },
    getStreet: function(id, isSet, name) {
      var self = this,
        ...
        title =
          name && config.shorthand ? name.replace(/街道|镇|乡/g, "") : name,
        converts = isNaN(title)
          ? "data-title=" + title
          : config.renderMode
          ? "data-id=" + title
          : 'value="' + title + '"',
      ...
      $.getJSON(config.streetUrl.replace("{json}", id), function(data) {
        ...
        placeholderStr = !config.renderMode
          ? '<option class="caller" value="">' +
            placeholder +
            "</option>" +
            effect.montage.apply(self, [reults, id])
          : '<li class="caller">' +
            placeholder +
            "</li>" +
            effect.montage.apply(self, [reults, id]);
        ...
        if (config.renderMode) {
          ...
          $street
            ...
            .eq(0)
            .remove();
          !isSet & !config.autoSelected
            ? $street.find(".reveal").addClass("df-color")
            : "";
        } else {
